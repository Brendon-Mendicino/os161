...
static int handle_swap_inc_page(struct swap_memory *swap, swap_entry_t entry)
{
    bool valid = true;

    spinlock_acquire(&swap->swap_lock);
    if (swap->swap_page_list[entry.val].refcount == 0)
        valid = false;
    else
        swap->swap_page_list[entry.val].refcount += 1;
    spinlock_release(&swap->swap_lock);

    return valid ? 0 : EINVAL;
}

static int handle_swap_dec_page(struct swap_memory *swap, swap_entry_t entry)
{
    bool valid = true;
...
static int handle_swap_get_page(struct swap_memory *swap, struct page *page, swap_entry_t entry)
{
    ...
    file_offset = (off_t)entry.val * PAGE_SIZE;
    ...
    handle_swap_dec_page(swap, entry);
